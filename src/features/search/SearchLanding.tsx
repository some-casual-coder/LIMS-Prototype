...
export function SearchLanding({ onRun }: { onRun: (q: string, mode: SearchMode) => void }) {
  ...
  return (
    <div className={styles.landing}>
        ...
        <Panel title="Recent research" icon={<Clock width={16} height={16} />}
          ...
          {myRecent.length === 0 ? (
            ...
            <ul className={styles.recentList}>
              {myRecent.map((r, i) => (
                <li key={r.id} className={`${styles.recentItem} item-in`} style={{ '--item-delay': `${i * 0.04}s` } as CSSProperties}>
          ...
          <Panel title="Saved searches" icon={<Bookmark width={16} height={16} />}
            ...
            {mySaved.length === 0 ? (
              ...
              <ul className={styles.savedList}>
                {mySaved.map((s, i) => (
                  <li key={s.id} className={`${styles.savedItem} item-in`} style={{ '--item-delay': `${i * 0.04}s` } as CSSProperties}>
      ...
      {/* Browse the repository — full-width launcher grid */}
      <Panel title="Browse the repository" icon={<Library width={16} height={16} />}
        ...
        <ul className={styles.collections}>
          {repositoryCollections.map((c, i) => (
            <li key={c.id} className="item-in" style={{ '--item-delay': `${i * 0.04}s` } as CSSProperties}>
